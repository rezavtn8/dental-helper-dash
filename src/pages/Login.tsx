...
import { Loader2, Shield, Users, ArrowLeft, Eye, EyeOff, Mail } from 'lucide-react';
...
export default function Login() {
  ...
  const { signInWithEmail, user, userProfile, signInWithGoogle } = useAuth();
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      // Success handling will be managed by the auth state change
    } catch (error) {
  ...
  const roleConfig = {
    owner: {
      icon: Shield,
      title: 'Sign In',
      subtitle: 'Welcome back to your clinic dashboard',
      color: 'text-primary',
      showGoogle: true,
      showSetup: true
    },
    assistant: {
      icon: Users,
      title: 'Sign In',
      subtitle: 'Access your task dashboard and clinic tools',
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          <IconComponent className={`w-12 h-12 mx-auto mb-4 ${config.color}`} />
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex items-center justify-center space-x-1 bg-muted/50 rounded-lg p-1">
            <Button
              type="button"
              variant={roleHint === 'owner' ? 'default' : 'ghost'}
              size="sm"
            ...
            <Button
              type="button"
              variant={roleHint === 'assistant' ? 'default' : 'ghost'}
              size="sm"
