-- Create comprehensive atomic clinic creation function
...
BEGIN
  ...
  SELECT EXISTS(
    SELECT 1 FROM public.clinics WHERE clinic_code = p_clinic_code
  ...
  SELECT EXISTS(
    SELECT 1 FROM public.users WHERE id = p_owner_id AND clinic_id IS NOT NULL
