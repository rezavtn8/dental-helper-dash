...
-- Add invitation_type enum for different invitation flows
CREATE TYPE IF NOT EXISTS public.invitation_type AS ENUM ('email_signup', 'magic_link');
...
-- Create function to generate secure magic link URLs
CREATE OR REPLACE FUNCTION public.generate_invitation_url(invitation_token text)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
...
AS $$
BEGIN
  RETURN format('https://jnbdhtlmdxtanwlubyis.supabase.co/auth/v1/magiclink?token=%s&type=invite&redirect_to=%s',
    invitation_token,
    encode(convert_to('/join?from=magic', 'UTF8'), 'base64'));
END;
$$;
...
BEGIN
  ...
  IF p_invitation_type = 'magic_link' THEN
    new_url := generate_invitation_url(new_token);
  ELSE
